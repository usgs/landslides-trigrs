...
TRIGRS, version 2.1.00a, ?? March 2012 (public version for TRIGRS 2.1)



  This distribution includes source code and executable files for the program TRIGRS and three companion utility programs, TopoIndex, GridMatch, and UnitConvert.  Executable files are available for Windows/XP/Vista/7 and for Mac OSX version 10.6 or higher.  The code can be compiled for Unix, Linux or other platforms that support Fortran 90/95 and Fortran 77.  
...
DOCUMENTATION
  ...
  The documentation for TRIGRS is contained in the text of U.S. Geological Survey Open-File Report 2008-1159, which is saved in PDF format as "OF20081159.pdf". The documentation can be viewed using Adobe Acrobat Reader.  This documentation includes a user guide and tutorial.  Additional information is available in 
...
EXTRACTING AND INSTALLING THE FILES
  ...
  To use the PC version, download the file "OF20081159_W.zip" to any convenient folder on the hard disk and double click on its icon to extract the files.  Use the dialog to select the desired target folder and click "OK" to extract the TRIGRS folder and its contents into the target location. The documentation can be downloaded and copied to the folder "doc" within the "TRIGRS" folder.  The PC version of TRIGRS and its companion programs require Windows XP or later and compatible hardware.  Memory and disk requirements vary as explained in the documentation.



  To use the Mac version, download the file "OF20081159_M.zip" to any convenient folder on the hard drive, preferably within your "home" folder (/Users/your_username).  Double click on the icon to extract the files.  Double-click the icon of the folder, "TRIGRS_2" to see the contents.  The documentation can be downloaded and copied to the folder "Documentation" within the TRIGRS_2 folder. The Mac version of TRIGRS and its companion programs require Mac OS X, version 10.6 or later and run on the command line.  See OF20081159.pdf the "User interface" section of this readme file for details.  Memory and disk requirements vary as explained in the documentation.
...
UnitConvert
  ...
  Use UnitConvert at the beginning of a project if you need to convert the data in a grid from one system of units to another.  UnitConvert reads the contents of an input grid file, multiplies each grid value by a conversion factor, and saves the result to the output grid file.  Input and output grids have the same resolution.  UnitConvert uses an initialization file and one input file of data in ASCII grid format; it produces one output file of data, also in ASCII grid format.  The initialization file contains three lines of input data that contain the name of the input file, the name of the output file and the conversion factor.  A descriptive header appears above each line of input data to aid the user.
...
Rex Baum baum@usgs.gov 
  ...
  We distribute updates from time to time as we make improvements or become aware of errors in the programs.
...
VERSION HISTORY
  ...
  TRIGRS 2.1.01a ?? March 2012 contains minor revisions to correct bugs in the previous public release.  It also contains several new output file options to support viewing of the computed water table and pressure head distribution.  
   These revisions resulted in changes to the input file "tr_in.txt."  (1) We added a user option to save grid files with the extension ".asc" as an alternative to ".txt".  ASCII Grid files with the .asc extension can be read directly into certain 2-D/3-D data visualization programs.  (2) We added a user option to ignore negative pore pressures (treat as zero) in computing factor of safety above the water table where pore pressure has been computed by the saturated infiltration model.  These two options appear near the end of the file "tr_in.txt."  (3) Output format of floating point grids was modified to make no-data values consistent between the file header and body, by adding a trailing decimal point in the header.  (4) Main subroutine names for infiltration model called by TRIGRS now appears in the log file.  (5) Several new optional output files were added.  The first adds the relative saturation (Chi) to the list file of pressure head and factor of safety with depth for each grid cell.  The second is an ijz listing of pore pressure and relative saturation by depth for each grid cell identified by column (i) and row (j) for each time step.  This addition requires a grid of topographic elevation.  The path of the elevation grid file now appears directly after the path of the slope grid in the file "tr_in.txt"  The third type is the XMDV file format, which saves xyz coordinates along with values of pressure head and degree of saturation.  This format can be imported by VisIt (https://wci.llnl.gov/codes/visit/) to enable three-dimensional visualization of the computed pressure head and saturation values.  Both the ijz and XMDV files come in three levels of detail: full depth profile, down-sampled depth profile (user specified increment between points), and a sparse  profile containing pressure at the ground surface, base, water table depth and two additional points representing the farthest excursions from linear projections joining the water table and surface/base points.  Both also allow the user to specify a pressure head value for a deep point to be used by the USGS program SCOOPS, which uses the method of columns to compute factor of safety for three-dimensional geometry.  The fourth new output option is ascii grid output of the computed water table elevation or depth.  (6) We made corrections to the code that computes unsaturated basal flux to eliminate certain errors (and associated error messages) that occurred during early-time computations.  We also made minor changes to reduce the time needed to compute the basal flux.  (7) We corrected formulas for pressure head when the initial condition uses pressure head =-1/alpha as the basal boundary condition.  (8) We added an option to ignore the height of capillary rise in determining how to compute pressure head when the unsaturated infiltration model is selected.  The program will over-ride this option to avoid early-time errors in the unsaturated solution. (9) We simplified input needed to import ascii grid files used for computation.  TRIGRS now reads the grid dimensions (rows and columns and counts data cells) directly from the elevation grid rather than requiring the user to provide these values. Public version of TRIGRS 2.1. 

  TRIGRS 2.0.06 19 Aug. 2009, contains minor revisions to (1) work around an error in the output from the saturated infiltration model that results in zero pressure-head output when multiple timesteps with zero surface infiltration occur in succession.  Pressure head from the previous timestep is now substituted in the output.  (2) A user option to offset the transient surface infiltration by the amount of the steady background flux was added.  This eliminates the increase of suctions beyond the initial conditions during storm sequences that contain time steps with surface infiltration (or rainfall) that is less than the steady background flux.  This option adds an input value to the last line of tr_in_txt.
...
TRIGRS 2.0.00  27 May 2008, initial public version of TRIGRS 2.0 
  ...
  TopoIndex 1.0.10 14 April 2011, Minor revisions to (1) add error trapping routine for flow-direction grid values that are out of range, (2) simplify input data by reading grid dimensions directly from input grid, (3) adding a line of code that was inadvertently omitted from previous versions.  The omission caused an error in the downslope cell output list when the D-8 method is used. (4) Added support for an output grid file to help identify points on ridge crests. Version of TopoIndex for use with TRIGRS 2.1.

  TopoIndex 1.0.03, 27 May 2008, changed the file extension of output grids from ".asc" to ".txt".  Improved handling of "nodata" values to allow different nodata values for integer and floating point grids (Thanks to Manfred Thüring, Institute of Earth Sciences -SUPSI, Ticino, Switzerland for pointing out this problem).  Also converted the subroutine nxtcel() to Fortran 90/95 to improve its loop structure. Served as Beta Version of TopoIndex for use with TRIGRS 2.0

  GridMatch 1.0.02, 18 April 2011, Minor revision to save a small file with grid dimensions that can be read by TRIGRS v 2.1.  Version of GridMatch for use with TRIGRS 2.1

  GridMatch 1.0.00, 27 May 2008, version of GridMatch for use with TRIGRS 2.0

  UnitConvert 1.0.02 27 May 2008, version of UnitConvert for use with TRIGRS 2.0 & 2.1
