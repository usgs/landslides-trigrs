...
write ( * ,  * ) '---------------------------------------'
write ( * ,  * ) '  HDF5 Added by Leon Foks 2019'
write ( * ,  * ) ''
tb = char(9)
...
101 continue
  ...
  if (outp(3)) then
      call writeHDF5File(nout, row, col, imx1, imax, fsmin, pf12D, test1, trim(folder) // trim(fminfil) // trim(suffix) // ".h5")
  endif
  if (outp(4)) then
      call writeHDF5File(nout, row, col, imx1, imax, zfmin, pf12D, test1, trim(folder) // trim(zfminfil) // trim(suffix) // ".h5")
  endif
  if (outp(5)) then
      call writeHDF5File(nout, row, col, imx1, imax, pmin, pf12D, test1, trim(folder) // trim(pminfil) // trim(suffix) // ".h5")
  endif
