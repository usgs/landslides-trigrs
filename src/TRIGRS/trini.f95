...
!  output file selections	
	...
	read (uini,*,err=420) outp(7) ! Save grid files of unsaturated zone basal flux?
 	...
 	read (uini,'(a)',err=420) heading; linct=linct+1
  	heading=adjustl(heading)
  	read (uini,*,err=423) flag,spcg! Save listing of pressure head and factor of safety ("flag") & increment (spcg)? ! Added spcg 2/14/2012 RLB
	  linct=linct+1	
  	write (ulog,*) trim(heading)
...
! added 2-15/2012 RLB ----
 	read (uini,'(a)',err=420) heading; linct=linct+1
  	heading=adjustl(heading)
	read (uini,*,err=420) igcapf ! Ignore height of capilary fringe greater than depth to water table in selecting infiltration model? 
	  linct=linct+1	
  	write (ulog,*) trim(heading)
  	write (ulog,*) igcapf
