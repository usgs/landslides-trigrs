...
!
subroutine svgstp_p(u1,rikf,ulog,i,rf,nccs,lcvs,nmx)
  ...
  temporal_loop: do m=1,nts+1 
        ...
        if(z<dusz1)then ! Revised 23 Oct 2013, RLB
           ...
           temporal_loop_1: do mm=1,nper
              ...
              if(tdif1 > 0.0) then
                 ...
                 term1=0.0
                 if(tfac1>late_t) then  !Added later time solution to improve performance and
                    !                        eliminate nonconvergent cells, 2/18-23/2015, RLB
                    ...
                    series_a_lt: do n=1,mmax ! later time solution
                       ...
                       ! test for convergence of series to within 1/1.e+06 of previous value	      
                       t1old=term1
                       tol=abs(term1/1.e+06) ! tol=term1/10000. revised 2/17/2015, RLB
                       term1=term1+flt1
              ...
              if(tdif2 > 0.0) then
                 ...
                 if(tfac1>late_t) then
                    ...
                    series_b_lt: do n=1,mmax ! later time solution
                       ...
                       ! test for convergence of series to within 1/1.e+06 of previous value	      
                       t2old=term2
                       tol=abs(term2/1.e+06) ! tol=term2/10000. revised 2/17/2015, RLB
                       term2=term2+flt2
     ...
     if (jf>0) then  ! revised 4/29/2010 to include pmin() RLB
        ...
        if(lpge0 .and. pmn<0.) then !option added 4/15/2010
           ...
           pmin(i+(jf-1)*isct(myrank)-idsp(myrank))=pmn
        end if
     end if
  end do temporal_loop
  return
end subroutine svgstp_p
